...
let rec string_of_value = function
  ...
  | VCustom(type_name, fields) ->
     let rec print_fields fields = match fields with
       | f::r when r = [] -> string_of_value f ^ (print_fields r)
       | f::r -> string_of_value f ^ ", " ^ (print_fields r)
       | [] -> ""
     in
     type_name ^ " { " ^ (print_fields fields) ^ " } "
  | VArray (name, env) ->
      let size =
