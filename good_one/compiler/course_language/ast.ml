...
type instruction =
  ...
  | Affect_custom of string * ((string * expr) list) * Annotation.t
  | Custom_decl of string * string * (string list) * Annotation.t
...
let rec pp_instruction fmt = function
  ...
  | Affect_custom(_) -> failwith("todo")
  | Custom_decl(_, _, _, _) -> failwith("todo")
...
let instruction_get_annotation = function
  ...
  | Print_expr (_, a) -> a
  | Var_decl (_, _, a) -> a
  | Affect_custom(_, _, a) -> a
  | Custom_decl(_, _, _, a) -> a
