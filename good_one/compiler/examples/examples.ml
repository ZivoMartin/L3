...
let execute_example name arguments =
  ...
  | Expression e ->
      ...
      let customs = Util.Environment.new_environment () in
      Format.printf "@[<v 0>Initial Environment:@,%a@,"
        Abstract_machine.pp_value_environment memory;
      let res = Interpreter.interpret_expr memory functions customs e in
  ...
  | Instruction i ->
      ...
      Format.printf "@[<v 0>Initial Environment:@,%a@,"
        Abstract_machine.pp_value_environment memory;
      Interpreter.interpret_instruction memory functions customs i;
